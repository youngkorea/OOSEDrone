...
int main(int argc, char **argv) {
    ...
    FILE *csv_file = fopen("benchmark_results_aoose_1000.csv", "w");
    if (csv_file) { 
    ...
    // Plaintext : U(G1) + v(Zr) + ID(Str) + m'(Zr) + r'(Zr) + m(Zr) + r(Zr) + CH(Str)
    ...
    printf("=== Start AOOSE measurements (Iteration: %d times) ===\n", ITERATIONS);
    ...
    for (int iter = 0; iter < ITERATIONS; iter++) {
        ...
        element_add(IBS_U, tmp1, tmp2); // U = v * SID_A + k * P
        ...
        // Verify OffSigncrypt Output (Authentication)
        pairing_apply(e1, IBS_U, P, pairing);
        pairing_apply(e2, QID_A, Ppub, pairing);
        element_pow_zn(e2_v, e2, IBS_v);
        ...
        int sig_valid_on = !element_cmp(IBS_v, IBS_v_rec);
        ...
        element_random(m_actual); // arbitrary message: 20 bytes
        ...
        offset += id_len;
        offset += element_to_bytes(plaintext + offset, m_prime); 
        offset += element_to_bytes(plaintext + offset, r_prime); 
        ...
        if (iter == ITERATIONS - 1) {
            ...
            if (dec_ret > 0 && sig_valid_on && sig_valid_un) {
                printf("-> Verification of AES-GCM, Authentication(On), and Signature(Un) passed!\n\n");
            } else {
    ...
    printf("=== AOOSE Benchmark Result (Average CPU time) ===\n");
    ...
    if (csv_file) { 
        ...
        fclose(csv_file);
        printf("-> Benchmark results are stored in 'benchmark_results_aoose.csv'.\n\n"); 
    }
